...
if [ "$1" = "download-all" ]; then
    ...
    FORMAT="${4:-auto}"  # Default to auto, or specify "h5ad" or "seurat"
    
    [ -z "$EXP_ID" ] && echo "Usage: $0 download-all <experiment-id> [output-dir] [h5ad|seurat]" && exit 1
    ...
    # Determine format
    if [ "$FORMAT" = "auto" ]; then
        echo "⚠️  Auto-detection disabled. Please specify format:" >&2
        echo "    $0 download-all $EXP_ID $OUTPUT_DIR h5ad" >&2
        echo "    $0 download-all $EXP_ID $OUTPUT_DIR seurat" >&2
        exit 1
    elif [ "$FORMAT" = "h5ad" ]; then
        FORMAT_EXT="h5ad"
        echo "Format: h5ad" >&2
    elif [ "$FORMAT" = "seurat" ]; then
        FORMAT_EXT="rds"
        echo "Format: seurat (.rds)" >&2
    else
        echo "Error: Invalid format '$FORMAT'. Use 'h5ad' or 'seurat'." >&2
        exit 1
    fi
    ...
    echo "=== Downloading processed matrix (${FORMAT}) ===" >&2
    "$SCRIPT_PATH" "$EXP_ID" "$FORMAT" "${OUTPUT_DIR}/${EXP_ID}_processed_matrix.${FORMAT_EXT}"
    echo "" >&2
...
if [ "$#" -lt 1 ]; then
    ...
    echo "  $0 download-all <experiment-id> [output-dir] [h5ad|seurat]" >&2
    ...
    echo "" >&2
    echo "Examples:" >&2
    echo "  $0 download-all exp123 ./data h5ad" >&2
    echo "  $0 download-all exp123 ./data seurat" >&2
    exit 1
fi
...
if [ "$API_TYPE" = "direct" ]; then
    ...
    PAYLOAD=$(jq -n \
        ...
        --arg fileName "${EXPERIMENT_ID}_processed_matrix.${FILE_EXT}" \
        '{
          experimentId: $exp,
          body: {
            name: $name,
